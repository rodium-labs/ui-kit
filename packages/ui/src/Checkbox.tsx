import { type InputHTMLAttributes, type ReactNode, useId } from 'react'
import { cn } from './cn'
import { Check } from './glyphs'

export interface CheckboxProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'type' | 'size'> {
  label?: ReactNode
  hint?: ReactNode
}

// the native box does the state and the keyboard; it is made invisible rather
// than removed, so the mark below is only ever a picture of it.
export function Checkbox({ label, hint, className, id, ...rest }: CheckboxProps) {
  const auto = useId()
  const boxId = id ?? auto

  return (
    <div className={cn('flex items-start gap-3', className)}>
      <span className="relative flex size-5 shrink-0 items-center justify-center">
        <input
          {...rest}
          id={boxId}
          type="checkbox"
          className="peer absolute inset-0 z-1 cursor-pointer opacity-0 disabled:cursor-not-allowed"
        />
        <span
          aria-hidden="true"
          className={cn(
            'flex size-5 items-center justify-center rounded-[6px] border border-night-edge bg-night-wash text-on-accent',
            'transition-[background-color,border-color,color] duration-(--motion-fast) ease-rl',
            'peer-hover:border-night-edge-lit',
            'peer-checked:border-brand-green peer-checked:bg-brand-green',
            'peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-(--color-brand-green)',
            'peer-disabled:opacity-45',
            'motion-reduce:transition-none',
          )}>
          <Check
            size={12}
            className="scale-75 opacity-0 transition-[opacity,scale] duration-(--motion-fast) ease-rl peer-checked:scale-100 peer-checked:opacity-100 motion-reduce:transition-none"
          />
        </span>
      </span>
      {label ? (
        <span className="flex flex-col gap-0.5">
          <label
            htmlFor={boxId}
            className="cursor-pointer text-[14px] leading-5 text-ink-on-night">
            {label}
          </label>
          {hint ? <span className="text-[12px] text-ink-on-night-dim">{hint}</span> : null}
        </span>
      ) : null}
    </div>
  )
}
